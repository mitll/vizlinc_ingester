...
class AcrossDocLocCoref extends CorefBase {

    AcrossDocLocCoref(VizLincDB graphdb) {
        super(graphdb)
    }
    ...
    static main(args) {
        Gremlin.load()
        if (args.size() != 1) {
            println "Usage: AcrossDocLocCoref.groovy <db_name>"
            System.exit(1)
        }
        def db = new VizLincDB(args[0])
        AcrossDocLocCoref coref = new AcrossDocLocCoref(db)
    ...
    void findCoreferences() {
        ...
        def nodes = db.getEntitiesOfType(E_LOCATION)
        ...
        mergeClusters(strong_clusters, 'across_doc_location_coref')
        mergeClusters(weak_clusters, 'weak_across_doc_location_coref')
