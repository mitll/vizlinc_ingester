...
class AcrossDocPerCoref extends CorefBase {

    AcrossDocPerCoref(VizLincDB graphdb) {
        super(graphdb)
    }
    ...
    static main(args) {
        Gremlin.load()
        if (args.size() != 1) {
            println "Usage: AcrossDocPerCoref.groovy <db_name>"
            System.exit(1)
        }
        def db = new VizLincDB(args[0])
        AcrossDocPerCoref coref = new AcrossDocPerCoref(db)
    ...
    void findCoreferences() {
        // Tunable parameters
        final NUM_TOKENS_MIN = 2
        final NUM_TOKENS_MAX = 6
        final NUM_CHARS_MIN = 5
        ...
        def nodes = db.getEntitiesOfType(E_PERSON)
        ...
        if (DEBUG) {
            ...
            outputClusters(strong_clusters)
            println "\nWeak clusters:"
            outputClusters(weak_clusters)
        }
        ...
        mergeClusters (strong_clusters, 'across_doc_person_coref')
        mergeClusters (weak_clusters, 'weak_across_doc_person_coref')
